@extends('layouts.admin')
@section('title', 'Kelola User')
@section('breadcrumb', 'KELOLA USER')

@section('content')

<div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
    <div>
        <h5 style="color:#fff;font-weight:800;margin:0;">Kelola User</h5>
        <p style="color:var(--pw-text-dim);font-size:13px;margin:0;">
            Daftar seluruh pengguna terdaftar di platform PortWatch AI.
        </p>
    </div>
    <a href="{{ route('admin.users.create') }}" class="btn-adm-primary">
        <i class="bi bi-person-plus"></i> Tambah User
    </a>
</div>

{{-- Filter --}}
<div class="adm-card mb-4">
    <form method="GET" action="{{ route('admin.users.index') }}" class="d-flex gap-3 flex-wrap align-items-end">
        <div style="flex:1;min-width:200px;">
            <label class="adm-form-label">CARI NAMA / EMAIL</label>
            <input type="text" name="search" class="adm-form-control"
                   placeholder="Ketik nama atau email..." value="{{ request('search') }}">
        </div>
        <div>
            <label class="adm-form-label">ROLE</label>
            <select name="role" class="adm-form-control" style="width:140px;">
                <option value="">Semua Role</option>
                <option value="user"  {{ request('role') === 'user'  ? 'selected' : '' }}>User</option>
                <option value="admin" {{ request('role') === 'admin' ? 'selected' : '' }}>Admin</option>
            </select>
        </div>
        <div class="d-flex gap-2">
            <button type="submit" class="btn-adm-primary">
                <i class="bi bi-search"></i> Cari
            </button>
            <a href="{{ route('admin.users.index') }}" class="btn-adm-outline">
                <i class="bi bi-x"></i> Reset
            </a>
        </div>
    </form>
</div>

{{-- Table --}}
<div class="adm-card">
    <div class="adm-section-title">
        <i class="bi bi-people me-2 text-cyan"></i>
        DAFTAR USER
        <span style="margin-left:auto;font-size:10px;font-family:'JetBrains Mono',monospace;color:var(--pw-text-dim);">
            {{ $users->total() }} DATA
        </span>
    </div>

    @if($users->isEmpty())
        <div style="text-align:center;padding:48px;color:var(--pw-text-dim);">
            <i class="bi bi-people" style="font-size:40px;display:block;margin-bottom:12px;opacity:.3;"></i>
            Tidak ada user ditemukan.
        </div>
    @else
    <div style="overflow-x:auto;">
        <table class="adm-table">
            <thead>
                <tr>
                    <th>#</th>
                    <th>NAMA</th>
                    <th>EMAIL</th>
                    <th>ROLE</th>
                    <th>BERGABUNG</th>
                    <th>AKSI</th>
                </tr>
            </thead>
            <tbody>
                @foreach($users as $user)
                <tr>
                    <td style="color:var(--pw-text-dim);font-family:'JetBrains Mono',monospace;font-size:12px;">
                        {{ $users->firstItem() + $loop->index }}
                    </td>
                    <td>
                        <div style="font-weight:600;color:#fff;">{{ $user->name }}</div>
                        @if($user->id === Auth::id())
                            <span style="font-size:10px;color:#f59e0b;">(Anda)</span>
                        @endif
                    </td>
                    <td style="color:var(--pw-text-dim);font-size:13px;">{{ $user->email }}</td>
                    <td>
                        <span class="{{ $user->role === 'admin' ? 'badge-admin' : 'badge-user' }}">
                            {{ strtoupper($user->role) }}
                        </span>
                    </td>
                    <td style="color:var(--pw-text-dim);font-size:12px;font-family:'JetBrains Mono',monospace;">
                        {{ $user->created_at->format('d/m/Y H:i') }}
                    </td>
                    <td>
                        <div class="d-flex gap-2">
                            <a href="{{ route('admin.users.edit', $user) }}" class="btn-adm-outline">
                                <i class="bi bi-pencil"></i> Edit
                            </a>
                            @if($user->id !== Auth::id())
                            <form method="POST" action="{{ route('admin.users.destroy', $user) }}">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn-adm-danger btn-confirm-delete">
                                    <i class="bi bi-trash3"></i>
                                </button>
                            </form>
                            @endif
                        </div>
                    </td>
                </tr>
                @endforeach
            </tbody>
        </table>
    </div>

    <div class="mt-4">
        {{ $users->links() }}
    </div>
    @endif
</div>

@endsection
